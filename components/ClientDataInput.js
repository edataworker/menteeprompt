...
export default function ClientDataInput({ clientData, setClientData }) {
  ...
  const parseSCORERawData = (text) => {
    ...
    // Extract key fields from SCORE export
    const patterns = {
    ...
    // Try to extract business name from Account section
    const accountMatch = text.match(/Account\s*Name\s*[:]\s*(.+)/i);
    if (accountMatch) {
      result.businessName = accountMatch[1].trim();
    }
  ...
  const handleFileUpload = (e) => {
    ...
    if (!validExtensions.includes(fileExt)) {
      setUploadError('Please upload Excel (.xlsx, .xls), CSV, JSON, or TXT file.');
      return;
    }
  ...
  return (
    <div className="card">
      ...
      <div className="mb-4 p-4 bg-[#f5f8fa] rounded-lg border-2 border-dashed border-[#004696]">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-xs text-gray-400">
            Supports: Excel (.xlsx, .xls), CSV, JSON, TXT (SCORE exports)
          </p>
        </div>
      ...
      {/* Text Area for Raw Data - Mimics your VBA workflow */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-2">
          📝 Or Paste Raw Client Data
        </label>
        <textarea
          name="rawText"
          placeholder="Paste raw client data here (SCORE export, etc.)..."
