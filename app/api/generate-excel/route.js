...
export async function POST(request) {
  try {
    ...
    if (!selectedPrompts || selectedPrompts.length === 0) {
      return new Response(
        JSON.stringify({ error: 'No prompts selected.' }),
    ...
    // Add rows to sheet
    clientRows.forEach(row => clientSheet.addRow(row));
    ...
    libraryData.forEach(row => librarySheet.addRow(row));
    ...
    selectedPrompts.forEach((p, index) => {
      ...
      promptHeaderRow.font = { name: 'Tahoma', size: 14, bold: true, color: { argb: 'FFFFFFFF' } };
    ...
    return new Response(
      JSON.stringify({ error: error.message || 'Failed to generate Excel file' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
}

// --- Helper Functions (copied from your lib/excelGenerator.js) ---
